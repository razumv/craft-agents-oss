...
export class NullBrowserPaneManager implements IBrowserPaneManager {
  ...
  // -- Instance management --
  createInstance(_id?: string, _options?: { show?: boolean; ownerType?: 'session' | 'manual'; ownerSessionId?: string | null }): string { return unavailable('createInstance') }
  ...
  reload(_id: string): void { unavailable('reload') }
  stop(_id: string): void { unavailable('stop') }
  ...
  // -- Events (no-ops) --
  onStateChange(_cb: (info: BrowserInstanceInfo) => void): void {}
  onRemoved(_cb: (id: string) => void): void {}
  onInteracted(_cb: (id: string) => void): void {}
}
